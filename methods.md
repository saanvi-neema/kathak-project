...
- **Comparison** — not built yet, but scoped: teacher-vs-student mode. User uploads two videos (a teacher's performance + their own take on the same piece), the two get time-aligned (even though pacing won't match exactly), then compared action-by-action using the teacher's video as ground truth -- e.g. *"at 0:42, the teacher clapped twice, you only clapped once"* or *"at 1:10, the teacher did two stomps, you only did one."* Two real pieces needed to build this: (1) alignment between two performances that are similar but not frame-synced -- `fastdtw` has been sitting unused in requirements.txt since the original project setup, exactly for this; (2) detecting discrete events (a clap, a stomp) specifically, which doesn't exist yet but is a natural extension of existing motion-peak/audio-onset detection.
